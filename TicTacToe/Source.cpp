...
void PrintField(char field[], const int n, char player)
{
	system("CLS"); //Вызывает любую команду командной строки, точнее 
	//любую программу, можно запустить из командной строки
	cout << VERTICAL_SHIFT;
	for (int i = 6; i >= 0; i -= 3)
	{
...
void Move(char field[], const int n, char player)
{
	...
	do
	{
		...
		//\a (alert / alarm); //вывод звукового сигнала
...
void Check(char field[], const int n, char player)
{
	...
	if (field[0] == player && field[4] && field[8] == player)game_over = true;
	if (field[2] == player && field[4] && field[6] == player)game_over = true;
